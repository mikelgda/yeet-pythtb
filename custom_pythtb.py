...
class wf_array(object):
    ...
    *Parametric or irregular k-space grid grid*:
    ...
    def _berry_eval_one(self,kpt):
        return self._model.solve_one(kpt,eig_vectors=True)

    def solve_on_grid(self,start_k,cores=None):
        r"""
        ...
        #
        pool = mp.Pool(mp.cpu_count()) if not cores else mp.Pool(cores)
        if self._dim_arr==1:
            # don't need to go over the last point because that will be
            # computed in the impose_pbc call
            pts = [start_k[0]+float(i)/float(self._mesh_arr[0]-1) for i in range(self._mesh_arr[0]-1)]
            res = pool.map(self._berry_evals_one,kpts)
            pool.close()
            evals = np.array([elem[0] for elem in res])
            evecs = np.array([elem[1] for elem in res])
            for i in range(self._mesh_arr[0]-1):
                self[i] = evecs[i]
                if all_gaps is not None:
                    all_gaps[i,:] = evals[i,1:] - evals[i,:-1]
            # for i in range(self._mesh_arr[0]-1):
            #     # generate a kpoint
            ...
            n1 = self._mesh_arr[0]-1
            n2 = self._mesh_arr[1]-1
            kpts = [[start_k[0]+float(i)/float(n1),\
                         start_k[1]+float(j)/float(n2)] for i in range(n1) for j in range(n2)]
            res = pool.map(self._berry_eval_one,kpts)
            pool.close()
            num_ener = res[0][0].size
            wf_dim = res[0][1].shape
            evals = np.array([elem[0] for elem in res]).reshape((n1,n2,num_ener))
            evecs = np.array([elem[1] for elem in res]).reshape((n1,n2)+wf_dim)
            for i in range(n1):
                for j in range(n2):
                    self[i,j] = evecs[i,j]
                    if all_gaps is not None:
                        all_gaps[i,j,:] = evals[i,j,1:] - evals[i,j,:-1]
            # for i in range(self._mesh_arr[0]-1):
            ...
            n1 = self._mesh_arr[0]-1
            n2 = self._mesh_arr[1]-1
            n3 = self._mesh_arr[2]-1
            kpts = [[start_k[0]+float(i)/float(n1),\
                    start_k[1]+float(j)/float(n2),\
                    start_k[2]+float(k)/float(n3)] \
                        for i in range(n1) \
                            for j in range(n2) \
                                for k in range(n3)]
            res = pool.map(self._berry_eval_one,kpts)
            ...
            num_ener = res[0][0].size
            wf_dim = res[0][1].shape
            evals = np.array([elem[0] for elem in res]).reshape((n1,n2,n3,num_ener))
            evecs = np.array([elem[1] for elem in res]).reshape((n1,n2,n3)+wf_dim)
            for i in range(n1):
                for j in range(n2):
                    for k in range(n3):
                        self[i,j,k] = evecs[i,j,k]
                        if all_gaps is not None:
                            all_gaps[i,j,k,:] = evals[i,j,k,1:] - evals[i,j,k,:-1]
            # for i in range(self._mesh_arr[0]-1):
            #     for j in range(self._mesh_arr[1]-1):
            ...
            n2 = self._mesh_arr[1]-1
            n3 = self._mesh_arr[2]-1
            n4 = self._mesh_arr[3]-1
            kpts = [[start_k[0]+float(i)/float(n1),\
                                     start_k[1]+float(j)/float(n2),\
                                     start_k[2]+float(k)/float(n3),\
                                     start_k[3]+float(l)/float(n4)] \
                                         for i in range(n1) \
                                             for j in range(n2) \
                                                 for k in range(n3) \
                                                     for l in range(n4)]
            ...
            for i in range(n1):
                for j in range(n2):
                    for k in range(n3):
                        for l in range(n4):
                            self[i,j,k,l] = evecs[i,j,k,l]
                            if all_gaps is not None:
                                all_gaps[i,j,k,l,:] = evals[i,j,k,l,1:] - evals[i,j,k,l,:-1]
            # for i in range(self._mesh_arr[0]-1):
            #     for j in range(self._mesh_arr[1]-1):
            #         for k in range(self._mesh_arr[2]-1):
